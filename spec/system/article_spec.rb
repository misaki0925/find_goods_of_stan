...
RSpec.describe 'Article', type: :system do
      ...
      describe '一般ユーザー用記事の検索部分' do
        ...
        context 'メンバーの名前で検索' do
          ...
          context "複数該当記事がある時" do
            ...
            context "どれかしらの記事に複数名のメンバー名が表示されていない時" do
              xit '全ての該当する記事の表示に成功する' do
                expect(current_path).to eq(articles_path)
            ...
            context "どれかしらの記事に複数名のメンバー名が表示されている時" do
              let!(:article_with_members) { create(:article, :with_members) }
              before do
                @members = []
              ...
              xit '該当する記事の表示に成功する' do
                expect(current_path).to eq(articles_path)
                fill_in "メンバー",	with: @members.first
                click_on '検索'
                expect(current_path).to eq(articles_path)
                # expect(page).to have_content("#{@members.join("さん")}さん") ここ治したい
                expect(page).to have_content(@members.first)
                expect(page).to have_content(@members.last)
                expect(page).to_not have_content("#{@name}}さん")
                expect(page).to have_content( '詳細ページへ' )
              end
            end
  ...
  describe '一般ユーザー用記事の詳細画面' do
    ...
    context 'メンバーが複数いる時' do
      ...
      # expect(page).to have_content("#{@members}さん")こう書きたいけど配列がうまくいかない
      expect(page).to have_content(@members.first)
      expect(page).to have_content(@members.last)
  ...
  describe '詳細画面のTweetへのリンクボタンが正常に動く' do
    xit '詳細画面のTweetへのリンクボタンから正常にTwitterへ飛ぶ' do
